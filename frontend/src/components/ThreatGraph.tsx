...
const RELATIONSHIPS = ['ALL', 'CAUSES', 'LEADS_TO', 'USES', 'TARGETS', 'REQUIRES', 'EXPLOITS', 'PRODUCES', 'DETECTED_BY', 'COUNTERED_BY', 'CONNECTED_TO'];
    ...
    <div className={`${fullscreen ? 'fixed inset-4 z-40 bg-[#0b1016] p-4 shadow-2xl' : ''} flex min-h-[700px] flex-col gap-4`}>
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-gray-800 bg-surface px-3 py-2 text-xs">
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex items-center gap-2 text-gray-400"><span className="sr-only">Search graph</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search attack steps" className="w-48 rounded border border-gray-700 bg-background px-2.5 py-1.5 text-gray-200 outline-none focus:border-primary" /></label>
