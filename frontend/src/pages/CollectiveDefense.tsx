...
import { DatabaseZap, Send, Users } from 'lucide-react';
...
export default function CollectiveDefense() {
  ...
  async function analyze() {
    ...
    setError(null);
    try {
      setResult(await api.collective(JSON.parse(payload)));
    } catch (reason) {
    ...
    <div className="page-stack">
      ...
      <Panel title="Submit sanitized intelligence" description="The backend enforces TLP and package validation. Do not paste organization-sensitive data.">
        ...
        <div className="action-row">
          <button className="button button-secondary" disabled={loading} onClick={() => setPayload(JSON.stringify(SAMPLE_PACKAGE, null, 2))}>
            <DatabaseZap size={16} /> Load sample
          </button>
          <button className="button button-primary" disabled={loading || !payload.trim()} onClick={analyze}>
      ...
      {result ? (
        <>
          <div className="metric-grid">
            ...
            <MetricCard label="Observed techniques" value={result.coverage.collective_techniques.length} />
            <MetricCard label="Local coverage" value={`${result.coverage.local_coverage}%`} />
            <MetricCard label="Collective coverage" value={`${result.coverage.collective_coverage}%`} tone="good" />
          </div>
